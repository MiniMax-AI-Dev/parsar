...
func TestCreateCoreAgentAPI(t *testing.T) {
	r := chi.NewRouter()
	RegisterRoutesWithStore(r, stubRuntimeStore{})
	for _, body := range []string{`{"name":"New Agent","connector_type":"agents_api","system_prompt":"Be concise","config":{"model":"test-model"}}`, `{"name":"Dup","config":{"model":"test-model"}}`} {
		...
		r.ServeHTTP(res, req)
		if res.Code != http.StatusCreated {
			t.Fatalf("create Core Agent: %d %s", res.Code, res.Body.String())
		}
	}
	for _, body := range []string{`{"name":"Invalid","runtime":"sandbox"}`, `{"name":"Invalid","initial_capabilities":[]}`, `{"name":"Invalid","connector_type":"http"}`, `{"name":"Invalid","config":{"agent_kind":"opencode"}}`, `{"name":"Invalid","config":{}}`} {
		req := withTestUser(httptest.NewRequest(http.MethodPost, "/api/v1/workspaces/00000000-0000-0000-0000-000000000002/agents", strings.NewReader(body)))
		res := httptest.NewRecorder()
		r.ServeHTTP(res, req)
		if res.Code < 400 || res.Code >= 500 {
			t.Fatalf("accepted unsupported settings: %d %s", res.Code, res.Body.String())
		}
	}
}
