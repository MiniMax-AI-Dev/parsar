...
// BuildPromptForUserChoiceTimeoutCard renders the post-timeout card the
// outbound driver patches the original message into when neither the
// human nor the daemon-side watchdog resolved the ask within 10
// minutes. Grey status tag; mirrors the body text the daemon writes
// back into the agent so the human and the model see the same story.
func BuildPromptForUserChoiceTimeoutCard(title string, questions []PromptForUserChoiceCardQuestion) map[string]any {
