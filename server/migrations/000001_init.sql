...
--                **不再写入 init**
--       - 000006 project_agent_runtime_id: project_agents.runtime_id
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_projects_workspace_slug_active
  ON projects(workspace_id, slug)
  WHERE deleted_at IS NULL;

COMMENT ON TABLE  projects IS '项目表, 最小业务权限边界';
COMMENT ON COLUMN projects.workspace_id IS '所属 workspace';
COMMENT ON COLUMN projects.name         IS '项目展示名';
COMMENT ON COLUMN projects.slug         IS 'workspace 内项目标识';
COMMENT ON COLUMN projects.description  IS '项目描述, 展示用';
COMMENT ON COLUMN projects.status       IS '状态: active=活跃 / archived=已归档(只读)';
COMMENT ON COLUMN projects.config       IS '项目级配置';
COMMENT ON COLUMN projects.created_by   IS '创建人';
COMMENT ON COLUMN projects.deleted_at   IS '软删除时间戳; NULL=未删除';
COMMENT ON INDEX  uk_projects_workspace_slug_active IS '同一 workspace 下项目 slug 活跃唯一';
...
--   slack_bot          — Slack 每 workspace bot token（xoxb-…），按 metadata->>'team_id' 解析(ResolveSlackBotSecretByTeam)
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_project_agents_active
  ON project_agents(project_id, agent_id)
  WHERE deleted_at IS NULL;
...
-- 按 runtime_id 反查 "哪些 project_agent 绑了我",runtime 详情 / 删除前 in-use 检查用。
CREATE INDEX IF NOT EXISTS idx_project_agents_runtime_active
  ON project_agents(runtime_id)
  WHERE deleted_at IS NULL AND runtime_id IS NOT NULL;

COMMENT ON TABLE  project_agents IS 'Agent 与 Project 绑定表';
COMMENT ON COLUMN project_agents.id           IS '绑定记录 ID';
COMMENT ON COLUMN project_agents.workspace_id IS '所属 workspace';
COMMENT ON COLUMN project_agents.project_id   IS '所属 Project; Project 删则级联删绑定';
COMMENT ON COLUMN project_agents.agent_id     IS '引用的 Agent; Agent 删则级联删绑定';
COMMENT ON COLUMN project_agents.runtime_id   IS '显式绑定的 runtime; NULL=未绑定(dispatch 时报错引导用户去 agent 设置页选择)';
COMMENT ON COLUMN project_agents.status       IS 'Project 内 Agent 启用状态';
COMMENT ON COLUMN project_agents.config       IS 'Project 级 Agent 覆写配置';
COMMENT ON COLUMN project_agents.created_by   IS '操作绑定的人';
COMMENT ON COLUMN project_agents.deleted_at   IS '软删除时间戳; NULL=有效绑定';
COMMENT ON INDEX  uk_project_agents_active         IS '同一 Project 内 Agent 活跃绑定唯一';
COMMENT ON INDEX  idx_project_agents_agent_active  IS '按 Agent 反查有效 Project 绑定';
COMMENT ON INDEX  idx_project_agents_runtime_active IS '按 runtime 反查 project_agent 绑定(runtime 详情 / 删前 in-use 检查)';
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_agent_capabilities_project_agent_capability
  ON agent_capabilities(project_agent_id, capability_id);

CREATE INDEX IF NOT EXISTS idx_agent_capabilities_agent_active
  ON agent_capabilities(project_agent_id)
  WHERE enabled = TRUE;
...
COMMENT ON COLUMN agent_capabilities.project_agent_id      IS '所属 project_agent';
...
COMMENT ON INDEX  uk_agent_capabilities_project_agent_capability IS '同一 Agent capability 绑定唯一';
...
CREATE TABLE IF NOT EXISTS sandboxes (
  id                             uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id                   uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  project_agent_id               uuid REFERENCES project_agents(id) ON DELETE CASCADE,
  name                           text,
  cache_key                      text,
  ...
  CONSTRAINT sandboxes_bound_shape_check CHECK (
    allocation_status <> 'bound' OR (project_agent_id IS NOT NULL AND cache_key IS NOT NULL)
  )
);

CREATE UNIQUE INDEX IF NOT EXISTS uk_sandboxes_active_per_agent
  ON sandboxes(workspace_id, project_agent_id)
  WHERE allocation_status = 'bound'
    AND project_agent_id IS NOT NULL
...
COMMENT ON TABLE  sandboxes IS '统一沙盒实例表。记录 workspace 范围内所有 provider 沙盒(如 E2B): 预热池沙盒(allocation_status=pooled)、已绑定到 project_agent 的持久沙盒(bound)、以及历史终止行(released)。不存 envd_access_token / endpoint URL 等敏感运行态信息; 这些只在进程内存中保存。';
...
COMMENT ON COLUMN sandboxes.project_agent_id               IS '绑定的 project_agent; pooled 行为空, bound 行必填';
...
COMMENT ON COLUMN sandboxes.allocation_status              IS '归属状态: pooled=workspace 预热池可 claim / bound=已绑定到 project_agent / released=已释放历史行';
...
COMMENT ON CONSTRAINT sandboxes_bound_shape_check ON sandboxes IS 'bound 行必须有 project_agent_id 和 cache_key; pooled/released 可为空';
COMMENT ON INDEX  uk_sandboxes_active_per_agent            IS '同一 workspace 下, 每个 project_agent 同时最多只能有一个未 kill 的 bound 沙盒';
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_conversations_external_active
  ON conversations(workspace_id, project_id, platform, external_id, external_thread_id)
  WHERE deleted_at IS NULL
    AND platform <> ''
    AND external_id <> '';
...
COMMENT ON INDEX  idx_conversations_project_active IS '按 Project 查询活跃会话';
COMMENT ON INDEX  uk_conversations_external_active IS '外部会话映射活跃唯一';
...
CREATE INDEX IF NOT EXISTS idx_messages_project_time_active
...
COMMENT ON COLUMN messages.project_id      IS '所属 project';
...
COMMENT ON INDEX  idx_messages_project_time_active      IS '按项目时间查询消息';
...
CREATE INDEX IF NOT EXISTS idx_agent_runs_project_agent_time
  ON agent_runs(project_agent_id, created_at DESC);
...
COMMENT ON COLUMN agent_runs.project_id         IS '所属 Project';
...
COMMENT ON COLUMN agent_runs.project_agent_id   IS '本次 run 使用的 project_agent';
...
COMMENT ON INDEX  idx_agent_runs_project_status      IS '按 Project/status 查询 run';
COMMENT ON INDEX  idx_agent_runs_project_agent_time  IS '按 project_agent 时间查询 run';
...
CREATE INDEX IF NOT EXISTS idx_agent_run_events_project_time
  ON agent_run_events(project_id, occurred_at DESC);
...
COMMENT ON COLUMN agent_run_events.project_id   IS '所属 project';
...
COMMENT ON INDEX  idx_agent_run_events_project_time IS '按 Project 时间查询事件';
...
CREATE INDEX IF NOT EXISTS idx_agent_run_artifacts_project_time
  ON agent_run_artifacts(project_id, created_at DESC);
...
COMMENT ON COLUMN agent_run_artifacts.project_id    IS '所属 project';
...
COMMENT ON INDEX  idx_agent_run_artifacts_project_time IS '按 Project 时间查询产物';
...
CREATE INDEX IF NOT EXISTS idx_usage_logs_project_time
...
COMMENT ON COLUMN usage_logs.project_id    IS '所属 project';
...
COMMENT ON INDEX  idx_usage_logs_project_time IS '按 Project/time 查询用量';
...
CREATE INDEX IF NOT EXISTS idx_audit_records_project_time
  ON audit_records (project_id, occurred_at DESC)
...
COMMENT ON COLUMN audit_records.project_id   IS '所属 project';
...
COMMENT ON INDEX  idx_audit_records_project_time       IS '按 project 时间查询审计记录';
...
--   - agent CLI 写:  source='agent',   created_by=NULL,     agent_actor='<connector>:<projectAgentID>'
...
COMMENT ON COLUMN spec_fragments.agent_actor  IS 'agent 写入时记录 connector:projectAgentID; 人工创建时为空字符串';
...
-- user/project 级 memory 共用一张表,通过 scope 区分。
-- memory_type 4 类: user/feedback/project/reference,枚举集中在
-- server/internal/specmemory/types.go (MemoryType/*),DB 层不加 CHECK IN
-- 约束。仅保留 scope ↔ project_id 的结构性约束防数据腐烂:
-- scope='user' 时 project_id 必须为 NULL,scope='project' 时必填。
...
CREATE TABLE IF NOT EXISTS memories (
  ...
  updated_at      timestamptz NOT NULL,
  deleted_at      timestamptz,
  CONSTRAINT memories_scope_project_id_match_check
    CHECK ((scope = 'user'    AND project_id IS NULL)
...
COMMENT ON TABLE  memories IS 'user/project 级 memory,agent 自觉写入 + 用户事后审计';
COMMENT ON COLUMN memories.id              IS 'memory 内部 ID';
COMMENT ON COLUMN memories.scope           IS '作用域 (user/project); 取值由 specmemory.Scope 管理';
COMMENT ON COLUMN memories.user_id         IS 'memory 归属用户; scope=project 时也填,用于排重与审计';
COMMENT ON COLUMN memories.project_id      IS '所属 project; scope=user 时为 NULL,scope=project 时必填';
COMMENT ON COLUMN memories.memory_type     IS '类型 (user/feedback/project/reference); 取值由 specmemory.MemoryType 管理';
COMMENT ON COLUMN memories.title           IS '简短标题,可选';
COMMENT ON COLUMN memories.body            IS '主体内容';
COMMENT ON COLUMN memories.why             IS 'feedback/project 类推荐填写的动因说明';
...
COMMENT ON COLUMN memories.agent_actor     IS 'agent 写入时记录 connector:projectAgentID; 人工写入为空';
...
COMMENT ON INDEX  idx_memories_project_active    IS '按 project 列出未删除 project 级 memory';
