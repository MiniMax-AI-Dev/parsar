...
async fn run(allow_cancel: bool) -> Result<()> {
    ...
    let (client, manager) = runtime::Runtime::start(&root, binary).await?;
...
async fn exercise(
    ...
    client.observations.require_active(&thread_id, &turn_id)?;
    ...
    let observation = client
        .observations
        .validate(&thread_id, &turn_id, phase, workspace, &marker, &history)?;
    ensure!(
