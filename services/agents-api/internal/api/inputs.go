...
// @Description For environment none, atomically accepts text messages, cancellation and function results. Messages steer active work or start a queued Turn. The self_hosted profile accepts idle text-only batches, waits up to the original five-minute connection/admission deadline and returns 204 only after durable admission. Cancellation-only self_hosted batches use existing durable cancellation admission and return 204 without waiting for native exit; a new cancellation conflicts while a pre-Turn reservation is pending. Homogeneous tool_result-only self_hosted batches reuse existing scoped result admission and application receipts without creating a Turn or bypassing a pending reservation. Active steering and mixed self_hosted batches remain unsupported. HTTP expiry/cancellation use local 409 environment_input_expired/environment_input_cancelled errors; exact hosted failure mapping is unverified. Losing execution ownership returns 503. Disconnecting the waiting HTTP request does not cancel retained work or restart its deadline. Retry keys identify the whole ordered batch. Function output accepts text or ordered text/image parts subject to engine support; Claude SDK currently accepts text results only. Message images are not supported yet.
// @Tags Sessions
// @Accept json
// @Security BearerAuth
