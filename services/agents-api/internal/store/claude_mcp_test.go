...
func TestClaudeMCPWaitsForCapableRuntime(t *testing.T) {
	for _, prebound := range []bool{false, true} {
		t.Run(fmt.Sprint(prebound), func(t *testing.T) {
			...
			claudeSession(t, h, mcpWorkerConfiguration, prebound)
			claudeHeartbeat(t, h, true) // Earlier ready bundles lack the MCP feature.
			...
			caps := proto.AgentKindCapabilities{Streaming: true, Steering: true, DurableTurns: true, DurableInputReceipts: true, ExecutionControls: true, EnvironmentNone: true, SubagentControl: true, ToolObservations: true, MCPHTTPTools: true}
			...
			if server.ServerLabel != "tickets" || server.ServerURL != "http://127.0.0.1:9191/mcp" || server.AllowedTools == nil || len(*server.AllowedTools) != 0 || server.Required || server.BearerToken != nil || !prompt.DisableExecutionEnvironment || !prompt.DisableSubagents {
				t.Fatal("MCP configuration changed during dispatch", prompt)
			}
...
func TestClaudeMCPUnsupportedSnapshotRejectedBeforeClaim(t *testing.T) {
	for _, profile := range []string{"required", "selected credential", "reserved label"} {
		t.Run(profile, func(t *testing.T) {
			...
			case "selected credential":
				configuration, _ = mcpBearerWorkerConfiguration(t, h)
			case "reserved label":
