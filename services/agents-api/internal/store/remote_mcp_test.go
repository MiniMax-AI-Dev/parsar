...
func TestRemoteMCPRejectsSelectedCredentialBeforePublicWrites(t *testing.T) {
	...
	for _, explicit := range []bool{false, true} {
		for _, initial := range []bool{false, true} {
			...
			if response.Code != http.StatusBadRequest || !strings.Contains(response.Body.String(), "unsupported_or_invalid_configuration") {
				t.Fatal("selected credential was admitted", response.Code, response.Body)
			}
		}
	}
...
func TestRemoteMCPWorkerRequiresCombinationCapability(t *testing.T) {
	for _, bound := range []bool{false, true} {
		t.Run(map[bool]string{false: "selection", true: "bound"}[bound], func(t *testing.T) {
			h := newDispatchHarness(t)
			enableWorkerEnvironment(t, h)
			configuration := strings.Replace(mcpWorkerConfiguration, `"type":"none"`, `"type":"self_hosted","workspace_directory":"/remote"`, 1)
			...
			if len(servers) != 1 || servers[0].ServerLabel != "tickets" || servers[0].AllowedTools == nil || len(*servers[0].AllowedTools) != 0 || servers[0].BearerToken != nil {
