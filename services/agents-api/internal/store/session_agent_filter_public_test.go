...
func TestSessionAgentFilterOfficialClient(t *testing.T) {
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, python, "../../tests/official_session_agent_filter.py", server.URL, token, foreign, recovered.URL)
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("official Session Agent filtering: %v %s", err, out)
	}
	t.Log(string(out))
}
