	...
	env, _ := proto.NewEnvelope(proto.TypeHeartbeat, "", proto.HeartbeatPayload{
		...
		SupportedAgentKinds: []proto.SupportedAgentKind{
			...
			},
			{
				Kind:         "codex",
				Available:    true,
				Capabilities: proto.AgentKindCapabilities{Steering: true, MessageItems: true, ToolItems: true, ToolObservations: true, EnvironmentNone: true, WebSearchControl: true, TextVerbosity: true, SubagentControl: true},
			},
		},
	})
	raw, _ := jsonMarshal(env)
