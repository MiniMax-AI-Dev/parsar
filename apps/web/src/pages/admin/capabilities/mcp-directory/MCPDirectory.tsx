...
export function MCPDirectory({
  ...
  const [sort, setSort] = useState<DirectorySort>("popular")
  ...
  if (itemID) {
    return (
      <>
        {success ? <SuccessBanner success={success} onViewCapability={onViewCapability} onAddToAgent={onAddToAgent} /> : null}
        <DirectoryDetail
          item={selected}
          loading={detailQ.isLoading}
          error={detailQ.error}
      ...
      <div className="rounded-xl border border-line bg-surface px-5 py-4">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-line pt-4">
          ...
          <select aria-label={t("capabilities.mcpDirectory.filters.sort")} value={sort} onChange={(event) => setSort(event.target.value as DirectorySort)} className="h-8 rounded-md border border-line bg-surface px-2.5 text-sm text-fg-muted outline-none focus:border-line-strong">
            <option value="popular">{t("capabilities.mcpDirectory.sort.popular")}</option>
...
function SuccessBanner({ success, onViewCapability, onAddToAgent }: {
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-3 rounded-lg border border-line bg-surface px-4 py-3" role="status">
      ...
      <Button size="sm" onClick={() => onAddToAgent(success.capabilityID)}>{t("capabilities.mcpDirectory.actions.addToAgent")}</Button>
    </div>
  )
}
