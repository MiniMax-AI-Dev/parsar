...
**A Place Is Not An Object Rule.** 设置 named a place and hid three real objects inside it: a credential ledger, a usage report and an audit log, none of which is a setting, each reachable only by first going somewhere it does not belong. When a "section" turns out to hold things of different shapes, those things are pages, and the section was a folder someone put them in. Settings keeps what is genuinely the workspace's own — its name, its language, its auth providers — and the ledgers stand with the other ledgers. The same test applies inside a page: 当前运行策略 was four property rows of hard-coded literals — a path format, a filesystem location, a note that the runtime moved to the Agent, and the names of two other pages — and passed none of the three tests for an object. It was not instanceable (one, identical for every workspace, forever), not structured (four unrelated facts), and not useful (nothing on it could be read off or acted on). Its own description said what it was: a note about which controls do not exist yet. A rule about a field belongs on that field; the rest was navigation written as prose.
