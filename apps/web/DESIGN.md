...
- **Body / UI** (400, 14px, 20px): sidebar nav items, form copy, dialogs, the toast strip, the entry panel's one muted sentence.
...
**The Two Voices Rule.** Two voices speak in this console and they never share a tone: ours (a sentence, in the reader's language, in ink) and the machine's (a server string, an exit code, a payload). The machine's goes in a `MachineText` block, one tone step down. `ErrorState` and the toast both take them as separate props for this reason; passing a raw server message as `description` puts words in our mouth that we did not write.
...
### Messages (`ErrorState`, `InlineError`, `InlineNotice`, `MachineText`, `Toast`)
...
- **ErrorState:** the block form: 14px triangle, 13px/500 title, `description` (12px ink — our sentence), `detail` (the server's string, in a `MachineText` block that shrinks to its content and wraps at 52rem), the hint 12px muted, one `sm` outline retry button indented 24px; 16px vertical padding; no red box.
- **MachineText:** anything the machine wrote — a server message, a command, a payload, a log — in a paper-muted block: 6px radius, 8px padding, 12px mono ink, relaxed leading, wrapping, capped at 160px and scrolling (pass `max-h-*` to change the cap, `max-h-none` to remove it). The fill is the whole point: one tone step down from our own copy is how a reader tells which half of a message was written for them. No border, no colour, no label.
- **Toast (`ToastProvider` / `useToast`):** the console's one transient message, portalled to `body`, fixed 12px from the top and centred; paper, hairline, 8px radius, floating shadow, `pop-in` on arrival and `pop-out` on exit; 6px vertical padding, 12px sides (12px / 6px when it carries an action); a 14px tone glyph with 13px ink text, an optional `detail` in `MachineText` beneath it, and optional buttons to its right. Four seconds, seven for an error; hovering or focusing *pauses* the countdown and leaving resumes it with the time that was left, so a resting cursor cannot pin a message on screen. At most three stand at once — a fourth asks the oldest to leave rather than yanking it. Failures live in an `aria-live="assertive"` region and everything else in a polite one; the strips carry no role of their own, so nothing is announced twice. `key` replaces a live message instead of stacking a second one, and `persist` keeps it until dismissed.
...
- **LayoutPrompt:** draws nothing of its own. It is one persistent toast (`key: "layout-adjusted"`) carrying three `sm` buttons 8px apart — primary 保存, outline 临时, ghost 恢复 — raised while a width is unsaved and dismissed on any choice. It persists because it is a question, and a question that times out has answered itself. It used to draw its own strip at the toast's coordinate, so a confirmation could land on top of it and swallow the clicks; a queue cannot collide with itself.
