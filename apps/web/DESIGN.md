...
- **RailSection:** the rail's section, and the only one it has (`DetailSection` is this component). A 12px/500 head with an optional muted count and one right-aligned action, 2px above its content, sections 20px apart. It stays 12px on purpose: in a 384px panel the head is an eyebrow over a dense property list, and at 16px it would weigh more than the values it introduces.
...
- **Sections (`RailSection`):** 20px apart, 12px/500 ink head with an optional muted tabular count and one right-aligned action, 2px below the head. The Agent rail's `DetailSection` is an alias of it, so the rail cannot drift into two sizes of the same thing.
